...
def options():
    ...
    parser.add_argument("-m", "--mask", help="Input mask file.", required=False)
...
def main():
    ...
    # Read image
    infile = os.path.join('images',args.image)
    print(infile)
    img, path, filename = pcv.readimage(filename=infile)

    mask_infile = os.path.join('predictions',args.mask)
    print(mask_infile)
    mask_img, mask_path, mask_filename = pcv.readimage(filename=)
...
#    b_cnt = pcv.threshold.binary(gray_img=b, threshold=160, max_value=255, object_type='light')
    ...
    masked2 = pcv.apply_mask(img=img, mask=mask_img, mask_color='white')
